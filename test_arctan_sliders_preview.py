...
def update_plot(a0, a1, b0, a2, b1, a3, b2):
    x = np.linspace(min(test_x), max(test_x), 100)
    y = a0 + a1 * (x - b0) + a2 * np.sqrt(b1 + (x - b0)**2) - a2 * np.sqrt(b1 + b0**2) + a3 * np.arctan(b2 * (x - b0))
    ax.clear()
    ax.plot(x, y, label='Fitted Function')
    ax.plot(test_x, test_y, 'ro', label='Test Data')
    
    # Calculate and display RMSE
    rmse = np.sqrt(mean_squared_error(test_y, a0 + a1 * (test_x - b0) + a2 * np.sqrt(b1 + (test_x - b0)**2) - a2 * np.sqrt(b1 + b0**2) + a3 * np.arctan(b2 * (test_x - b0))))
...
def slider_update(val):
    ...
    a3 = slider_a3.get()
    b2 = slider_b2.get()
    update_plot(a0, a1, b0, a2, b1, a3, b2)
