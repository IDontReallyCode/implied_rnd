...
def main():
    ...
    for one_date in random_dates:
        ...
        this_pf = this_pf.filter(pl.col('eod')==True)        
        ...
        """
        this_pf = this_pf.filter((pl.col('dte') > 6) & (pl.col('bid') > 0)  & (pl.col('bid_iv') > 0)
                                 & ((pl.col('ask') - pl.col('bid')) / ((pl.col('ask') + pl.col('bid')) / 2) < 1.75) 
                                 & ((pl.col('ask') + pl.col('bid')) / 2 > 0.05)
